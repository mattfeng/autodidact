...
Use Python 3.11 or 3.12. This problem set is a `uv` project, and its dependencies are listed in `pyproject.toml`. From the project directory, the commands below create the environment and install CPU-compatible packages. A GPU is optional; all correctness checks must pass on CPU, though full training will be slower. If Python 3.12 is unavailable on your system, set `.python-version` to `3.11` before running `uv sync`.
...
The setup commands above are the only commands in this section that you should expect to run after the project scaffold is present and before implementing `src/`. Commands that invoke `src.*` modules, `scripts/*.py`, or `pytest` are correctness checks to run after you have created the provided files and implemented the files named in the relevant problem.
...
```bash
uv run python -m src.train --config configs/tiny.yaml
```
...
```bash
uv run python -m src.check_pretrained --checkpoint-dir checkpoints/openai-gpt2
```
...
You will edit every file under `src/`. The `pyproject.toml`, `.python-version`, and the files under `configs/`, `scripts/`, and `tests/` are instructor-provided support files: create them exactly as shown and use them unchanged. The `data/raw/`, `data/processed/`, `data/manifests/`, and `checkpoints/openai-gpt2/` contents are generated by commands in this problem set.
...
7. In `GPT2Tokenizer.__init__`, check that both `vocab_path` and `merges_path` exist. Raise `FileNotFoundError` with a message mentioning `uv run python scripts/download_openai_gpt2.py` if either file is missing.
...
```bash
uv run pytest -q tests/test_attention.py tests/test_model_shapes.py
```
...
- `uv run python -m src.generate --checkpoint-dir checkpoints/openai-gpt2 --prompt "In a shocking finding, scientists discovered" --max-new-tokens 40 --seed 1234` prints decoded text after you implement `src/generate.py` and download pretrained weights.
...
- `uv run python -m src.check_pretrained --checkpoint-dir checkpoints/openai-gpt2` exits successfully.
...
```bash
uv run pytest -q tests/test_weight_loading.py
uv run python scripts/download_openai_gpt2.py
uv run python -m src.check_pretrained --checkpoint-dir checkpoints/openai-gpt2
```
...
```bash
uv run python scripts/download_climbmix.py --dataset gvlassis/ClimbMix --split train --output-dir data/raw/climbmix --max-docs-per-cluster 2000 --seed 1234
uv run python scripts/download_openai_gpt2.py
uv run python scripts/build_climbmix_subset.py --input-dir data/raw/climbmix --output-dir data/processed/climbmix_gpt2_1m --manifest-dir data/manifests --tokens-per-cluster 50000 --block-size 1024 --val-fraction 0.01 --test-fraction 0.01 --seed 1234
uv run pytest -q
uv run python -m src.train --config configs/tiny.yaml
uv run python -m src.check_pretrained --checkpoint-dir checkpoints/openai-gpt2
```
...
```bash
uv run pytest -q tests/test_no_transformers.py
```
...
```bash
uv run python scripts/download_openai_gpt2.py
uv run pytest -q tests/test_tokenizer.py tests/test_data.py
```
...
```bash
uv run pytest -q tests/test_attention.py tests/test_model_shapes.py tests/test_loss.py
```
...
```bash
uv run python -m src.generate --checkpoint-dir checkpoints/openai-gpt2 --prompt "In a shocking finding, scientists discovered" --max-new-tokens 40 --seed 1234
```
